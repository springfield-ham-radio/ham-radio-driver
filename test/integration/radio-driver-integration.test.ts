...
class MockSerialPort extends EventEmitter {
    ...
    if (this.isMagicNumber(sentData)) {
      ...
      if (this.currentOperation === 'write') {
        ...
        if (address >= 0 && address <= 63) {
          segmentEndAddress = 63;
        } else if (address >= 128 && address <= 191) {
          segmentEndAddress = 191;
        } else {
  ...
  private isWriteSegment(data: Buffer): boolean {
    // Write chunk: 'X' + address (2 bytes) + chunkSize + payload
    if (data.length >= 4 && data[0] === 0x58) {
...
const baofengUV5RRadio: Radio = {
  ...
  version: '1.0.0',
  description: 'Baofeng UV-5R radio configuration for integration tests',
  settingsSchema: {
    model: 'baofeng-uv5r' as RadioModelId,
    settingsSchema: {},
    channelSchema: {},
